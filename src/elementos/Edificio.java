...
public class Edificio {
    ...
    public Edificio(Celda c1, String nombre, Civilizacion civil, int tipo, int salud1, int CRM, int CRP, int ataq, int def, int CCC, int capAlm, int capAlojar, int capPer) {
        ...
        } else if (tipo != Mapa.TCIUDADELA && tipo != Mapa.TCASA && tipo != Mapa.TCUARTEL) {
            this.estado = false;
            this.saludInicial = 0;
    ...
    public void setCelda(int x_ej, int y_ej) {
        if (x_ej >= 0 && y_ej >= 0) {
            this.celda = new Celda(x_ej, y_ej, this.tipoEdificio, this.nombre, this.civilizacion);
        }
    }
    ...
    private void elimina(Mapa m, Celda c) {
